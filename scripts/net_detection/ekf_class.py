...
class ExtendedKalmanFilter(object):
    def __init__(self, x0=[0, -1, 1]):  # x y d bei der gleichung point*n=d wobei n=[x y]'
        number_of_states = 3
        """ initialize EKF """
    ...
    def h(self, x, measurements, number_of_measurements):  # distance between 0 and 1
        ...
        z = x[0] * measurements[:, 0] + x[1] * measurements[:, 1] - x[2] * np.ones(
            (1, number_of_measurements))
        point_e = measurements[:,0:2] - np.transpose(x[0:2] * z)
        return point_e

    # Jacobian of the measurement function
    def h_jacobian(self, x, measurements, number_of_measurements):  # dim : number_of_measurements X 3 X 4
        h_jacobian = np.zeros((number_of_measurements, 2, 3))
        for i in range(number_of_measurements):
            h_1_n0 = -2 * measurements[i, 0] * x[0] - x[1] * measurements[i, 1]+ x[2]
            h_1_n1 = -x[0] * measurements[i, 1]
            h_1_d = x[0]

            h_2_n0 = -x[1] * measurements[i, 0]
            h_2_n1 = -2 * measurements[i, 1] * x[1] - x[0] * measurements[i, 0] + x[2]
            h_2_d = x[1]


            h_jacobian[i, 0, 0] = h_1_n0
            h_jacobian[i, 0, 1] = h_1_n1
            h_jacobian[i, 0, 2] = h_1_d

            h_jacobian[i, 1, 0] = h_2_n0
            h_jacobian[i, 1, 1] = h_2_n1
            h_jacobian[i, 1, 2] = h_2_d
    ...
    def update(self, measurements):  # z_meas, x_pos, y_pos):
        ...
        for i in range(number_of_measurements):
            ...
            self.__x_est = self.__x_est + np.matmul(k_mat, y_tild[i, :]).reshape(3, 1)  # = x_est + k * y_tild
